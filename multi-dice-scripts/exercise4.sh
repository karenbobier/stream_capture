...
echo "zeta=sample(1:8,1,replace=T)/8
tau1s=sample(1000:1000000,1,replace=T)
beta=round(exp(.95*log(tau1s))):round(exp(1.05*log(tau1s)))
NPOP0=sample(5000:1000000,8,replace=T)
NPOP1=sample(1500:15000,8,replace=T)
NM1=exp(sample(round((log(1e-5)*100000)):round((log(10)*100000)),8,replace=T)/100000)
MIG1=NM1/NPOP0
NM2=exp(sample(round((log(1e-5)*100000)):round((log(10)*100000)),8,replace=T)/100000)
MIG2=NM2/NPOP1
NM3=exp(sample(round((log(1e-5)*100000)):round((log(10)*100000)),8,replace=T)/100000)
MIG3=NM3/NPOP0
NM4=exp(sample(round((log(1e-5)*100000)):round((log(10)*100000)),8,replace=T)/100000)
MIG4=NM4/NPOP1
RESIZE0=exp(sample(round((log(.01)*100000)):round((log(100)*100000)),8,replace=T)/100000)
RESIZE1=exp(sample(round((log(.01)*100000)):round((log(100)*100000)),8,replace=T)/100000)
TIME2=c(rep(tau1s,(zeta*8)),sample(c(1000:1000000)[!c(1000:1000000)%in%beta],((1-zeta)*8),replace=T))
TIMEP=sample(100:99900,8,replace=T)/100000
...
rm -r dice.draws dice.hyperdraws dice.draw dice.par dice MRCAs.txt seed.txt


for b in {1..8}
do
cat dice.draws|cut -d ' ' -f 1 >dice.draw
cat dice.template.par|sed -e "s/NPOP0/`cat dice.draw|awk 'NR==1'`/g" -e "s/NPOP1/`cat dice.draw|awk 'NR==2'`/g" -e "s/MIG1/`cat dice.draw|awk 'NR==4'`/g" -e "s/MIG2/`cat dice.draw|awk 'NR==6'`/g" -e "s/MIG3/`cat dice.draw|awk 'NR==8'`/g" -e "s/MIG4/`cat dice.draw|awk 'NR==10'`/g" -e "s/RESIZE0/`cat dice.draw|awk 'NR==11'`/g" -e "s/RESIZE1/`cat dice.draw|awk 'NR==12'`/g" -e "s/TIME2/`cat dice.draw|awk 'NR==13'`/g" -e "s/TIME1/`cat dice.draw|awk 'NR==15'`/g" >dice.par
/home/keb27269/apps/fastsimcoal2/2.6.0.2/fsc26 -i dice.par -n 10000 -m -c 1 --multiSFS >/dev/null 2>&1
cat dice/dice_MSFS.txt|awk 'NR==3' >>dice.SFS.1
done
